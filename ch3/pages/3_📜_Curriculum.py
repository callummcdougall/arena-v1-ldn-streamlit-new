...
def generate_fig():
    ...
    counter = -1
    path_list = ["pre", "trans", "opti", "laws", "mod", ("scale", "mod"), "rl", "int", "adv", "cap"]
    assert len(path_list) == len(days_per_section)
    for (days, path) in zip(days_per_section, path_list):
        if isinstance(path, str):
            ...
            path1 = f + f"{path[1]}.png"
            path2 = f + f"{path[0]}.png"
    ...
    df_day = []
    for url, day in zip(df["Exercises link"], df["Day"]):
        # Check if this is a two-path day
        if ", " in url:
            urls = url.split(", ")
            s = f"{day}: " + "<a href='{}'>4A</a>, <a href='{}'>4B</a>".format(*urls)
        else:
            s = f"<a href='{url}'>{day}</a>"
        df_day.append(s)
    df["Day"] = df_day
...
""")
    ...
    ch1_columns = st.columns(1)
    with ch1_columns[0]:
        st.markdown("""<h5><code>W3D3</code>&emsp;&emsp;&emsp;&emsp;&emsp;|&emsp; <a href="https://arena-ch3.streamlit.app/W3D3-5_-_Scaling_Laws_Investigations">Scaling Laws: investigations 📚</a></h5>""", unsafe_allow_html=True)
        st.markdown("""
---
    """)
...
""")
    ...
    ch1_columns = st.columns(1)
    with ch1_columns[0]:
        st.markdown("""<h5><code>W4D1</code> - <code>W4D2</code>&emsp;|&emsp; GANs 📝</h5> """, unsafe_allow_html=True)
        st.markdown("""
Learn how **GANs** (Generative Adversarial Models) work, and build & train some of your own.

---
    """)
        st.markdown("""<h5><code>W4D3 - W4D4</code>&emsp;&nbsp;|&emsp; VAEs 📝</h5> """, unsafe_allow_html=True)
        st.markdown("""
...
---
""")
        st.markdown("""<h5><code>W4D5 - W5D1</code>&emsp;&nbsp;|&emsp; Diffusion models 🔬</h5> """, unsafe_allow_html=True)
        st.markdown("""
...
""")
    ...
    st.markdown("""<h5><code>W4D3</code>&emsp;&emsp;&emsp;&emsp;&emsp;|&emsp; Docker 📝</h5> """, unsafe_allow_html=True)
    st.markdown("""
Today, you'll learn about Docker, and use it to deploy your own application containing a model and training loop.
""")

    st.markdown("""<h5><code>W4D4 - W4D5</code>&emsp;&nbsp;|&emsp; Diffusion models 📝</h5> """, unsafe_allow_html=True)
    st.markdown("""
...
""")
    ...
    st.markdown("""<h5><code>W5D1 - W5D2</code>&emsp;&nbsp;|&emsp; Diffusion models 🔬</h5> """, unsafe_allow_html=True)
    st.markdown("""
Over these next three days, you'll be learning about the maths behind diffusion models. Hopefully, you'll also get to build some of your own.
""")
